...
static const char *CONFIG_EXAMPLE_PEER_ADDR = "ADDR_ANY";
                 ...
                 void *arg)
{
    ...
    dsc = peer_dsc_find_uuid(peer,
                             BLE_UUID16_DECLARE(BLECENT_SVC_CURRENT_TIME_UUID),
                             BLE_UUID16_DECLARE(BLECENT_CHR_CURRENT_TIME_UUID),
...
blecent_should_connect(const struct ble_gap_disc_desc *disc)
{
    ...
    for (i = 0; i < fields.num_uuids16; i++) {
        if (ble_uuid_u16(&fields.uuids16[i].u) == BLECENT_SVC_CURRENT_TIME_UUID ) {
            return 1;
        }
    }
...
void aometer_setup_ble_client(void)
{
    int rc;
    ...
    rc = ble_svc_gap_device_name_set("ao-meter");
    assert(rc == 0);

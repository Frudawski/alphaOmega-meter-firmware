...
esp_err_t camera_init(const camera_config_t* config)
{
    ...
    // todo: add raw modus to camera init
    } else if (pix_format == PIXFORMAT_RAW) {
                s_state->fb_size = s_state->width * s_state->height * 1;
                s_state->sampling_mode = SM_0A0B_0C0D;
                s_state->dma_filter = &dma_filter_raw;
                s_state->in_bytes_per_pixel = 1;       // camera sends RAW 1x8bit (BGGR)
                s_state->fb_bytes_per_pixel = 1;       // frame buffer stores RAW 1x8bit (BGGR)
